    ...
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            ...
            if (!strcmp(opt, "zero-padding")) {
                median = (kernel_size * kernel_size-1)/2;
                count = 0;
...
Mat Salt_pepper_noise_removal_RGB(const Mat input, int n, const char *opt) {
    ...
    Mat kernel2= Mat::zeros(kernel_size * kernel_size, channel, input.type() - 16);
            ...
            if (!strcmp(opt, "zero-padding")) {
                ...
                for (int x = -n; x <= n; x++) { // for each kernel window
                    for (int y = -n; y <= n; y++) {
                        ...
                        */
                        if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)){
                            kernel.at<G>(count, 0) = input.at<Vec3b>(i + x, j + y)[0];
                            kernel.at<G>(count, 1) = input.at<Vec3b>(i + x, j + y)[1];
                            kernel.at<G>(count, 2) = input.at<Vec3b>(i + x, j + y)[2];
                        }else{
                            kernel.at<G>(count, 0) = 0;
                ...
                for (int x = -n; x <= n; x++) { // for each kernel window
                    for (int y = -n; y <= n; y++) {
                        ...
                        */
                        ...
                        kernel.at<G>(count,0) = input.at<Vec3b>(tempa, tempb)[0];
                        kernel.at<G>(count,1) = input.at<Vec3b>(tempa, tempb)[1];
                        kernel.at<G>(count,2) = input.at<Vec3b>(tempa, tempb)[2];
                        count++;
                    }
                }
            }
