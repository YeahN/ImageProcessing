...
int main() {
	...
	Mat Denoised_Gray = Bilateralfilter_Gray(noise_Gray, 3, 10, 3, 10, "zero-padding");
    Mat Denoised_RGB = Bilateralfilter_RGB(noise_RGB, 3, 10, 3, 10, "adjustkernel");
...
Mat Bilateralfilter_Gray(const Mat input, int n, double sigma_t, double sigma_s, double sigma_r, const char *opt) {
    ...
    for (int i = 0; i < row; i++)
        for (int j = 0; j < col; j++)
            for (int a = -n; a <= n; a++)
                for (int b = -n; b <= n; b++)
                    Wij = exp(-(pow((double)input.at<G>(i,j)-(double)input.at<G>(i+a,j+b), 2)) / 2 * pow(sigma_r, 2));
					...
					for (int y = -n; y <= n; y++) {
						...
						*/
                        ...
                        if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)){
                            w = exp(-(pow((double)input.at<G>(i,j)-(double)input.at<G>(i+x,j+y), 2)) / 2 * pow(sigma_r, 2));
                            sum += kernel.at<double>(x+n,y+n) * w;
                        }
					}
				}
                output.at<G>(i,j) = (G)sum * Wij;
			}
...
Mat Bilateralfilter_RGB(const Mat input, int n, double sigma_t, double sigma_s, double sigam_r, const char *opt) {
